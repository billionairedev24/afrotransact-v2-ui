...
export function Footer() {
  return (
    <footer className="bg-brand-dark text-white/80 border-t border-black/30 pb-[max(1rem,env(safe-area-inset-bottom,0px))] md:pb-0">
      ...
      <div className="mx-auto max-w-[1440px] px-4 sm:px-6 py-12">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-8">
          ...
          <div className="col-span-2 sm:col-span-3 md:col-span-1 space-y-4">
            ...
            <div className="flex items-center gap-2 pt-1">
              <a
                href="https://www.linkedin.com/company/afrotransact-usa"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="LinkedIn"
                className="flex h-8 w-8 items-center justify-center rounded-lg border border-white/20 text-white/70 hover:border-brand-gold hover:text-brand-gold transition-colors"
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
              </a>
              {/*<a  ToDo: Enable when tiktok is available
                ...
                {section.links.map((link) => (
                  <li key={link.label}>
                    {link.isStartSelling ? (
                      ...
                      <Link
                        href={link.href}
                        className="inline-flex items-center gap-1.5 text-[13px] text-white/70 hover:text-brand-gold transition-colors"
                      >
                        {link.label}
                        {link.comingSoon && (
                          <span className="rounded-full bg-white/10 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wider text-white/50">
                            Soon
                          </span>
                        )}
                      </Link>
        ...
        <div className="mt-10 pt-6 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-white/60">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/seller-agreement" className="hover:text-brand-gold transition-colors">
              Seller Agreement
            </Link>
