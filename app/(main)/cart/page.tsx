      ...
      <main className="mx-auto max-w-[1200px] px-4 sm:px-6 py-8">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          <div className="flex-1 space-y-5">
            <div className="rounded-2xl border border-border bg-card overflow-hidden h-48 animate-pulse" />
          </div>
          <aside className="lg:w-[340px] shrink-0">
            <div className="rounded-2xl border border-border bg-card p-5 h-64 animate-pulse" />
          </aside>
        </div>
      </main>
    )
  ...
  if (items.length === 0) {
    return (
      <main className="min-h-[70vh] flex flex-col items-center justify-center px-4 py-12">
        ...
        <p className="text-muted-foreground text-sm text-center max-w-xs leading-relaxed mb-1">
  ...
  return (
    <main className="mx-auto max-w-[1200px] px-4 sm:px-6 py-8">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
                ...
                <div className="divide-y divide-border">
                  {groupItems.map((item) => (
                    <div key={item.variantId} className="flex gap-3 sm:gap-4 p-4 sm:p-5">
                      <div
                        className="relative w-20 h-20 rounded-xl shrink-0 bg-muted flex items-center justify-center overflow-hidden"
                      >
                        {item.imageUrl ? (
                          <Image
                            src={item.imageUrl}
                      ...
                      <div className="flex-1 min-w-0 flex flex-col gap-2">
                        <div className="flex justify-between gap-3 items-start">
                          <div className="min-w-0">
                            ...
                            <p className="text-muted-foreground text-xs mt-0.5">{item.variantName}</p>
                            <p className="text-foreground font-semibold text-sm mt-1">
                              {formatCents(item.price)}
                            </p>
                          </div>
                          <p className="shrink-0 text-foreground font-semibold text-sm sm:hidden tabular-nums">
                            {formatCents(item.price * item.quantity)}
                          </p>
                        </div>

                        <div className="flex flex-wrap items-center gap-3">
                          <div className="flex items-center shrink-0 rounded-lg border border-border overflow-hidden touch-manipulation">
                            <button
                              type="button"
                              onClick={() => updateQuantity(item.variantId, item.quantity - 1)}
                              className="flex min-h-11 min-w-11 sm:min-h-0 sm:h-9 sm:w-9 items-center justify-center text-muted-foreground hover:bg-muted transition-colors"
                              aria-label="Decrease quantity"
                            >
                              <Minus className="h-4 w-4" />
                            </button>
                            <span className="min-w-[2.5rem] px-1 text-center text-sm text-foreground font-medium tabular-nums">
                              {item.quantity}
                            </span>
                            <button
                              type="button"
        ...
        <aside className="lg:w-[340px] shrink-0">
          <div
            className="rounded-2xl border border-border bg-card p-5 sticky top-[110px]"
          >
            ...
            {mounted && !eligibilityBlocked && !locationSet && storeIds.length > 0 && (
              <div className="mt-4 flex gap-2 rounded-xl border border-border bg-muted px-3 py-2.5 text-xs text-muted-foreground">
            ...
            <Link
              href="/"
              className="mt-3 w-full flex items-center justify-center text-sm text-muted-foreground hover:text-foreground transition-colors py-2"
            >
              Continue Shopping
            </Link>
