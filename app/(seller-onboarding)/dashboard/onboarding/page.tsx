      ...
      <div className="rounded-xl border border-yellow-500/20 bg-yellow-500/5 p-4">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm text-yellow-400">
            Click &quot;Connect Stripe Account&quot; above first. A secure card form will appear here once your account is ready.
          </p>
        </div>
      </div>
    )
...
function StripeCardForm({ onConfirm, saving }: { onConfirm: (pmId: string) => void; saving: boolean }) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
        ...
        >
          {confirming ? <><Loader2 className="h-4 w-4 animate-spin" /> Saving Card…</> : <>Save Payment Method</>}
        </button>
        <p className="text-[10px] text-gray-600 mt-3 flex items-center gap-1">
          <Shield className="h-3 w-3" />
          Card data is processed by Stripe and never touches our servers. PCI DSS compliant.
        </p>
      </div>
    </form>
  )
