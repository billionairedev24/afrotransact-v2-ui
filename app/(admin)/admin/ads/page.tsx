...
function AdModal({
    ...
    return (
      <div>
        ...
        <select
          value={String(form[key])}
          onChange={(e) => setForm({ ...form, [key]: e.target.value })}
          className="w-full rounded-xl border border-white/15 bg-[hsl(0_0%_11%)] px-4 py-2.5 text-sm text-white outline-none focus:border-primary/60 transition-colors"
        >
          {options.map((o) => (
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        className="relative rounded-2xl border border-white/15 w-full max-w-xl max-h-[90vh] overflow-y-auto p-6 space-y-4"
        style={{ background: "hsl(0 0% 10%)" }}
      >
        <div className="flex items-center justify-between sticky top-0 pb-2" style={{ background: "hsl(0 0% 10%)" }}>
          <h3 className="text-lg font-bold text-white">{isEdit ? "Edit Ad" : "New Ad"}</h3>
        ...
        {field("Slot ID (must match <AdSlot slotId=\"…\" /> in code)", "id", {
          ...
          disabled: isEdit,
          className: isEdit
            ? "w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-gray-500 outline-none cursor-not-allowed"
            : undefined,
        })}
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="flex items-end gap-3">
            ...
            <div className="flex-1">
              <label className="block text-xs text-gray-400 mb-1">Dismissible</label>
              <button
                type="button"
        ...
        <div>
          ...
          <div
            className={`rounded-xl border border-white/10 px-4 py-3 bg-gradient-to-r ${form.gradient}`}
          >
            {form.badgeText && (
            ...
            {form.body && <p className="text-xs text-gray-400 mt-0.5">{form.body}</p>}
          </div>
        </div>

        <div className="flex gap-3 pt-2">
          <button onClick={onClose} className="flex-1 rounded-xl border border-white/15 py-2.5 text-sm text-gray-300 hover:bg-white/5">
            Cancel
          </button>
          <button
            onClick={handleSave}
    ...
    <div className="space-y-6">
      ...
      {ads.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-white/15 py-16 text-center">
          ...
          <p className="text-gray-400 text-sm">No ads configured yet.</p>
          ...
          {ads.map((ad) => (
            <div
              ...
              className={`rounded-2xl border transition-colors ${
                ad.enabled ? "border-white/10 bg-white/3" : "border-white/5 bg-white/2 opacity-50"
              }`}
              style={{ background: ad.enabled ? "hsl(0 0% 10%)" : "hsl(0 0% 8%)" }}
            >
              {/* Preview strip */}
              ...
              <div className="px-4 py-3 flex items-center justify-between gap-4 flex-wrap">
                <div className="min-w-0">
                  <p className="text-xs text-gray-500 font-mono">slotId: <span className="text-white">{ad.id}</span></p>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {confirmDelete === ad.id ? (
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => { deleteAd(ad.id); setConfirmDelete(null) }} className="text-xs font-bold text-red-400 hover:text-red-300">Yes</button>
                      <button onClick={() => setConfirmDelete(null)} className="text-xs text-gray-400 hover:text-white">No</button>
                    </div>
                  ) : (
                    <button
                      onClick={() => setConfirmDelete(ad.id)}
                      className="flex items-center gap-1.5 rounded-lg border border-red-500/20 bg-red-500/5 px-3 py-1.5 text-xs font-medium text-red-400 hover:bg-red-500/15 transition-colors"
                    >
      ...
      <div className="rounded-xl border border-white/10 bg-white/3 px-5 py-4 text-xs text-gray-500 space-y-1" style={{ background: "hsl(0 0% 9%)" }}>
        <p className="font-semibold text-gray-400">How ad slots work</p>
        <p>Each ad has a <span className="font-mono text-white/60">slotId</span> that must match the <span className="font-mono text-white/60">&lt;AdSlot slotId=&quot;…&quot; /&gt;</span> placement in the page code.</p>
        <p>Current placement IDs in use: <span className="font-mono text-white/60">mid-page-1</span>, <span className="font-mono text-white/60">mid-page-2</span>, <span className="font-mono text-white/60">bottom-strip</span>.</p>
