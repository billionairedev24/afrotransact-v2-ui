        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr><td colSpan={9} className="px-4 py-16 text-center"><Loader2 className="h-5 w-5 animate-spin text-gray-400 mx-auto" /></td></tr>
              ) : coupons.length === 0 ? (
                <tr>
                ...
                coupons.map(c => (
                  <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${c.active ? "bg-emerald-50 text-emerald-600" : c.enabled ? "bg-yellow-50 text-yellow-600" : "bg-gray-100 text-gray-500"}`}>
                        {c.active ? "Active" : c.enabled ? "Expired" : "Disabled"}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <button onClick={() => handleToggle(c.id)} className="rounded-lg p-1.5 text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors" title={c.enabled ? "Disable" : "Enable"}>
                          {c.enabled ? <ToggleRight className="h-4 w-4 text-emerald-500" /> : <ToggleLeft className="h-4 w-4" />}
                        </button>
