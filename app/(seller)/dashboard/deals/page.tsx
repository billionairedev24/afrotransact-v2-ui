...
import {
  Loader2, AlertCircle, Plus, Tag, MoreHorizontal, Pencil,
  Trash2, ToggleLeft, ToggleRight, Clock, Eye,
} from "lucide-react"
import { toast } from "sonner"
import {
  getSellerDeals, createSellerDeal, updateSellerDeal,
  deleteSellerDeal, toggleSellerDeal, getSellerProducts,
  type DealData, type DealCreateRequest, type Product,
} from "@/lib/api"
...
const EMPTY_FORM: DealCreateRequest = {
  productId: "", title: "", description: "", badgeText: "", discountPercent: undefined, dealPriceCents: undefined,
}
...
export default function SellerDealsPage() {
  ...
  const [form, setForm] = useState<DealCreateRequest>(EMPTY_FORM)
  ...
  function openEdit(d: DealData) {
    setEditing(d)
    setForm({
      productId: d.productId, title: d.title, description: d.description || "",
      badgeText: d.badgeText || "", discountPercent: d.discountPercent ?? undefined,
      dealPriceCents: d.dealPriceCents ?? undefined, startAt: d.startAt || undefined, endAt: d.endAt || undefined,
    })
    setShowForm(true)
  }
  ...
  async function handleSave() {
    const token = await getAccessToken()
    if (!token || !form.title || !form.productId) return
    setSaving(true)
    try {
      if (editing) {
        await updateSellerDeal(token, editing.id, form)
        toast.success("Deal updated")
      } else {
        await createSellerDeal(token, form)
    ...
    <div className="max-w-[960px] mx-auto px-4 sm:px-6 py-8 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Create promotions for your products to attract more customers.</p>
        </div>
      ...
      <div className="flex items-start gap-3 rounded-xl border border-[#EAB308]/20 bg-[#EAB308]/5 p-4">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1 leading-relaxed">
            Create a deal for any of your active products. Set a discount percentage or fixed deal price, and optionally schedule start/end dates.
            Active deals appear on the homepage and deals page, distributed evenly across all sellers.
          </p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white">
        ...
        {deals.length === 0 ? (
          ...
          <>
            <div className="hidden sm:grid grid-cols-[1fr_120px_100px_100px_90px_44px] gap-2 px-5 py-2.5 text-xs text-gray-500 font-medium uppercase tracking-wide border-b border-gray-100">
              <span>Deal</span><span>Product</span><span>Price</span><span>Status</span><span className="text-right">Ends</span><span />
            </div>
            <div className="divide-y divide-gray-100">
              {deals.map((d) => (
                <div key={d.id} className="grid grid-cols-1 sm:grid-cols-[1fr_120px_100px_100px_90px_44px] gap-2 px-5 py-3 items-center hover:bg-gray-50 transition-colors">
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">{d.title}</p>
                    {d.badgeText && <span className="text-[10px] text-primary font-semibold">{d.badgeText}</span>}
                  </div>
                  <span className="text-xs text-gray-500 truncate">{d.productTitle || d.productId.substring(0, 8)}</span>
      ...
      <Sheet open={showForm} onClose={() => setShowForm(false)}>
        ...
        <SheetBody>
          <div className="space-y-4">
            ...
            <div>
              ...
              <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-primary" placeholder="e.g. Spring Sale — 20% Off!" />
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input value={form.badgeText || ""} onChange={e => setForm({ ...form, badgeText: e.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-primary" placeholder="e.g. HOT, BOGO, 20% OFF" />
              </div>
              <div>
        ...
        <SheetFooter>
          ...
          <button onClick={handleSave} disabled={saving || !form.title || !form.productId} className="rounded-xl bg-primary px-5 py-2 text-sm font-bold text-black hover:bg-primary/90 transition-colors disabled:opacity-50">
